...
export default function BuyerOrder() {
  ...
  const releaseMutation = useMutation({
    mutationFn: async () => {
      const hex = getBuyerPrivateKeyHex(token);
      if (!hex) {
        throw new ApiError(
          "BUYER_KEY_MISSING",
          "We can't find the secret release key in this browser. " +
            "Open this order on the device you used to pay — your secret key " +
            "stays on that device for your security.",
        );
      }
      return apiClient.releaseOrder(token, { buyerPrivateKeyHex: hex });
    },
    onSuccess: (res) => {
      ...
      >
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-5">
            ...
            {/* Photos upload placeholder — wired in a follow-up commit
                when the dispute-evidence Blossom flow is built. */}
            <div className="space-y-2">
              <Label className="text-sm font-semibold text-ink">
                Add photos (optional)
              </Label>
...
function getTimelineState(status: ApiOrderStatus): TimelineStep[] {
  ...
  const stateByStep: Record<ApiOrderStatus, TimelineStepState[]> = {
    ...
    payment_locked: ["done", "active", "pending", "pending"],
  ...
  return (
    <section className="rounded-2xl border border-brand-soft/60 bg-brand-soft/60 p-5 sm:p-6">
      ...
      <ul className="mt-4 space-y-2 text-sm">
        <BankRow
          label="Bank"
          value={order.bitnobBank ?? "—"}
          onCopy={() => copy(order.bitnobBank ?? "", "Bank")}
        />
        <BankRow
          label="Account number"
          value={order.bitnobAccount ?? "—"}
          onCopy={() => copy(order.bitnobAccount ?? "", "Account number")}
          highlight
        />
        <BankRow
          label="Account name"
          value="SafeSale Escrow"
          onCopy={() => copy("SafeSale Escrow", "Account name")}
        />
        <BankRow
          label="Amount"
        ...
        <p className="mt-4 flex items-center gap-1.5 text-xs text-ink-soft">
          ...
          <span>
            ...
            <span className="font-medium tabular-nums">
              {formatCountdownLong(order.expiresAt)}
            </span>
            . Use your normal banking app — no special instructions needed.
          </span>
        </p>
      )}
      <p className="mt-2 text-xs leading-relaxed text-ink-soft">
        Once your transfer is confirmed (usually under 60 seconds), this page
        will update automatically and your payment will be locked in escrow.
      </p>
    </section>
  );
}
    ...
    <section className="rounded-2xl border border-border bg-background p-5 sm:p-6">
      ...
      {!locked && countdownIso && (
        <p className="mt-3 text-xs leading-relaxed text-ink-soft">
          Releasing payment is final. Funds go to the seller's Lightning wallet
          and the transaction completes. Auto-release in{" "}
...
function CompletedReviewPlaceholder() {
  // The full review-publish flow (kind 1985) ships as a follow-up
  // commit; for now show a friendly "thanks" panel so the page isn't
  // empty after release.
  return (
