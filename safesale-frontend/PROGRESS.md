...
| 4 | Seller Orders (list + detail) | `/app/orders` & `/app/orders/:token` | Seller | 🟡 | List view (`src/pages/app/OrdersPage.tsx`) wired to `useSellerOrders` — agrees with the dashboard. Detail page (`src/pages/app/OrderDetailPage.tsx`) wired: `apiClient.getOrder` (8s poll) + Mark Shipped → `apiClient.shipOrder`. **Stitch prompt drafted (`.stitch-designs/04-seller-orders.prompt.md`); awaiting Stitch HTML, then React port.** |
...
| 4 — Seller Orders (list + detail) | `.stitch-designs/04-seller-orders.prompt.md` | _(awaiting Stitch)_ |
